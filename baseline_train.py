...
class BTS(nn.Module):
    ...
    def __init__(self, pretrained: bool = True, feat_ch: int = 256):
        ...
        self.lpg8_head = nn.Conv2d(feat_ch // 2, 4, 1)
        ...
        self.lpg4_head = nn.Conv2d(feat_ch // 4, 4, 1)
        ...
        self.lpg2_head = nn.Conv2d(feat_ch // 8, 4, 1)
...
def save_scene_video(snaps, epoch, out_dir):
    ...
    for snap in snaps:
        ...
        fig.suptitle(f"{scene}  frame {snap['idx']}  |  MAE={snap['mae']:.4f}  "
                     f"RMSE={snap['rmse']:.4f}  δ<1.05={snap['delta1']:.2%}  |  epoch {epoch}",
                     fontsize=12)
        fig.canvas.draw()
...
def save_training_plots(history, plot_dir, model_name):
    ...
    ax.set_xlabel("Epoch"); ax.set_ylabel("Accuracy (%)"); ax.set_title("Val δ < 1.05")
...
def run_epoch(loader, model, optimizer, scaler, device, *, is_train, epoch,
...
def parse_args():
    ...
    p.add_argument("--lr", type=float, default=3e-4)           # == LR
    p.add_argument("--weight-decay", type=float, default=1e-4) # == AdamW wd
    p.add_argument("--warmup-epochs", type=int, default=0)     # == warmup_epochs
...
def main():
    ...
    # Optimizer + schedule mirror train.py: AdamW + (optional) linear warmup
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        val_loss, _, val_metrics, val_snaps = run_epoch(
            val_loader, model, optimizer, scaler, device,
